/**
 * Single-session scan script for cross-session receipt reproducibility.
 * Run as a separate Node process: npx tsx scripts/phase-4c-a3-single-session.ts <N>
 */
...
async function main() {
  console.error(`=== Session ${sessionNum} (PID ${process.pid}) ===`);
  console.error(`Started at: ${new Date().toISOString()}`);

  const server = createServer();
  const client = new Client(
    { name: `session-${sessionNum}`, version: '1.0.0' },
  ...
  const sessionPath = path.join(EVIDENCE_DIR, `session-${sessionNum}.json`);
